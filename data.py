...
def _upload_evidence_to_storage(storage_path: str, file_bytes: bytes, mime_type: str) -> bool:
    """Upload file evidence ke Supabase Storage. True kalau berhasil."""
    ...
    try:
        ...
        print(f"[SIMONA] Gagal upload evidence ke Storage: {e}")
        return False


def get_evidence_signed_url(storage_path: str, expires_in: int = 3600) -> str | None:
    """Buat URL sementara untuk preview/download evidence dari Supabase Storage."""
    client = _get_supabase_client()
    if not client or not storage_path:
        return None
    try:
        res = client.storage.from_(_EVIDENCE_BUCKET).create_signed_url(storage_path, expires_in)
        return res.get("signedURL") or res.get("signed_url") or res.get("signedUrl")
    except Exception as e:
        print(f"[SIMONA] Gagal membuat signed URL evidence: {e}")
        return None
...
def add_evidence(unit_id, period_id, indicator_id, filename, uploaded_by, file_bytes=None, mime_type=None):
    ...
    storage_path = f"{unit_id}/{period_id}/{indicator_id}/{_new_id()}_{filename}"
    uploaded_ok = _upload_evidence_to_storage(storage_path, file_bytes, mime_type) if file_bytes else False
    ...
    if uploaded_ok:
        # mode Supabase aktif: file beneran ada di Storage, cuma simpan path-nya
        evidence_entry["storage_path"] = storage_path
    else:
        # fallback kalau Supabase belum di-setup: simpan bytes langsung di memori
        # (TIDAK permanen — hilang kalau server restart, dan tidak disarankan
        # untuk pemakaian nyata karena bikin data membengkak)
        evidence_entry["file_bytes"] = file_bytes

    val.setdefault("evidences", []).append(evidence_entry)
    _persist()
