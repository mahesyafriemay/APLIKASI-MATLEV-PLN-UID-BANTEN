...
from data import (
    ...
    get_all_credentials_list, reset_demo_data,
)
...
with tab_backup:
    ...
    st.divider()
    with st.expander("Lihat daftar ID & password semua unit"):
        st.caption("Hanya kamu (UID) yang bisa lihat ini. Jangan sebar screenshot halaman ini sembarangan.")
    ...
    with st.expander("Reset semua data ke kondisi awal (hati-hati)"):
        st.error(
            "Ini akan MENGHAPUS seluruh assessment, evidence, dan perubahan yang sudah dibuat "
            "SEMUA unit, lalu mengembalikan ke data awal (Juni 2026 real + Juli 2026 kosong). "
            "Tidak bisa dibatalkan kecuali kamu punya file backup."
        )
        confirm_reset = st.checkbox("Saya yakin ingin menghapus semua data dan reset ke kondisi awal.")
        if st.button("Reset Sekarang", disabled=not confirm_reset, use_container_width=True):
            reset_demo_data()
